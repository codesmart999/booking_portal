			...
			$.post(apiUri, formData, function (data) {
				...
				arr_bookingperiods.forEach(objBookingPeriod => {
                    if (arr_from_bookings.hasOwnProperty(objBookingPeriod.FromInMinutes + "-" + objBookingPeriod.ToInMinutes)) {
                        const arrBookingInfo = arr_from_bookings[objBookingPeriod.FromInMinutes + "-" + objBookingPeriod.ToInMinutes];

                        for (const objBookingInfo of arrBookingInfo) {
                            ...
                            const checkbox = $('<input>').attr({
                                type: 'checkbox',
                                id: 'chk_from_booking_' + objBookingPeriod.id + '_' + objBookingInfo.booking_code,
                                name: 'chk_from_booking[]',
                                value: objBookingInfo.booking_code,
                                class: 'chk_from_booking chk_from_booking_' + objBookingInfo.booking_code
                            });
                            ...
                            const label = $('<label>').attr({
                                for: 'chk_from_booking_' + objBookingPeriod.id + '_' + objBookingInfo.booking_code,
                                class: 'timeslot'
                            }).text(objBookingPeriod.DisplayText);//DisplayText
                            if (objBookingInfo)
                                label.addClass("booked");
                            
                            cell2.append(label);

                            const span1 = $('<span>').text(" " + objBookingInfo.business_name)
                                .css('margin-left', '5px')
                                .css('background-color', generateBackgroundColor(objBookingInfo.booking_code));
                            cell2.append(span1);
                            const span2 = $('<div>').text(" (" + objBookingInfo.booking_code + ")");
                            cell2.append(span2);
                        ...
                        const checkbox = $('<input>').attr({
                            type: 'checkbox',
                            id: 'chk_from_booking_' + objBookingPeriod.id,
                            name: 'chk_from_booking[]',
                            value: ""
                        });
                        ...
                        const cell2 = $('<td>');
                        const label = $('<label>').attr({
                            for: 'chk_from_booking_' + objBookingPeriod.id,
                            class: 'timeslot'
                ...
                $(".chk_from_booking").change(function() {
                    var booking_code = $(this).val();
                    $(".chk_from_booking_" + booking_code).prop('checked', $(this).prop('checked'));
                })
	        });
        }
			...
			$.post(apiUri, formData, function (data) {
				...
				arr_to_all_bookingperiods.forEach(objBookingPeriod => {
                    if (arr_to_bookings.hasOwnProperty(objBookingPeriod.FromInMinutes + "-" + objBookingPeriod.ToInMinutes)) {
                        const arrBookingInfo = arr_to_bookings[objBookingPeriod.FromInMinutes + "-" + objBookingPeriod.ToInMinutes];

                        for (const objBookingInfo of arrBookingInfo) {
                            if (arrBookingCodes.indexOf(objBookingInfo.booking_code) != -1)
                                continue;

                            arrBookingCodes.push(objBookingInfo.booking_code);
                            ...
                            const checkbox = $('<input>').attr({
                                type: 'radio',
                                id: 'chk_to_booking_' + objBookingInfo.booking_code,
                                name: 'chk_to_booking',
                                value: objBookingInfo.booking_code
                            });
                            ...
                            const cell2 = $('<td>');
                            const label = $('<label>').attr({
                                for: 'chk_to_booking_' + objBookingInfo.booking_code,
                                class: 'timeslot'
                            }).text(arr_to_bookings[objBookingInfo.booking_code][2]);//DisplayText
                            if (objBookingInfo)
                                label.addClass("booked");
                            
                            cell2.append(label);

                            const span = $('<span>').text(" " + objBookingInfo.business_name + " (" + objBookingInfo.booking_code + ", " + (arr_to_bookings[objBookingInfo.booking_code][1] - arr_to_bookings[objBookingInfo.booking_code][0]) + " mins)");
                            cell2.append(span);
